...
double ray_plane_intersection(double* Ro, double* Rd, double D, double* n){
	return -(n[0]*Ro[0] + n[1]*Ro[1] + n[2]*Ro[2] + D) / (Rd[0] + Rd[1] + Rd[2]);
	//Ray: R(t) = Ro + Rd*t
	//Plane: ax + by + cz + d = 0
	//solve for t
...
double ray_sphere_intersection(double* Ro, double* Rd, double* c, double r){
	// Ray: P = Ro + Rd*t
...
void raycast(Scene scene, char* outfile, PPM fileinfo){
